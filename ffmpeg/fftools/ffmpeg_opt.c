...
const OptionDef options[] = {
    ...
    { "vt_remote_transcode", OPT_TYPE_BOOL, OPT_VIDEO | OPT_PERSTREAM | OPT_EXPERT | OPT_OUTPUT,
        { .off = OFFSET(vt_remote_transcode) },
        "enable vtremote transcode bitstream filter for stream copy", "0|1" },
    { "vt_remote_out_codec", OPT_TYPE_STRING, OPT_VIDEO | OPT_PERSTREAM | OPT_EXPERT | OPT_OUTPUT,
        { .off = OFFSET(vt_remote_out_codec) },
        "vtremote transcode output codec (h264|hevc)", "codec" },
    { "vt_remote_pix_fmt", OPT_TYPE_STRING, OPT_VIDEO | OPT_PERSTREAM | OPT_EXPERT | OPT_OUTPUT,
        { .off = OFFSET(vt_remote_pix_fmt) },
        "vtremote transcode pixel format (1=nv12,2=p010)", "fmt" },
    { "vt_remote_port", OPT_TYPE_STRING, OPT_VIDEO | OPT_PERSTREAM | OPT_EXPERT | OPT_OUTPUT,
        { .off = OFFSET(vt_remote_port) },
        "vtremote transcode server port", "port" },
    { "vt_remote_out_width", OPT_TYPE_STRING, OPT_VIDEO | OPT_PERSTREAM | OPT_EXPERT | OPT_OUTPUT,
        { .off = OFFSET(vt_remote_out_width) },
        "vtremote transcode output width", "width" },
    { "vt_remote_out_height", OPT_TYPE_STRING, OPT_VIDEO | OPT_PERSTREAM | OPT_EXPERT | OPT_OUTPUT,
        { .off = OFFSET(vt_remote_out_height) },
        "vtremote transcode output height", "height" },
    { "vt_remote_scale_mode", OPT_TYPE_STRING, OPT_VIDEO | OPT_PERSTREAM | OPT_EXPERT | OPT_OUTPUT,
        { .off = OFFSET(vt_remote_scale_mode) },
        "vtremote transcode scale mode (stretch|aspect|aspect_fill)", "mode" },
